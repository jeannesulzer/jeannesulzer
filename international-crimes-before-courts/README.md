...
The original export is preserved as [`international-crimes-before-courts-data.xlsx`](international-crimes-before-courts-data.xlsx). Each sheet is also available in machine-readable form under [`data/`](data/), as both CSV (original column headers) and JSON (snake_case keys, empty fields omitted).

| Dataset | Rows | Description |
|---|---|---|
| [`cases`](data/cases.csv) | 198 | National & hybrid prosecutions of international crimes — the main corpus |
| [`judicial-decisions`](data/judicial-decisions.csv) | 140 | Individual court decisions (indictments, judgments, appeals, supreme-court rulings, provisional measures, arrest warrants, investigations) nested under each case |
...
**Layer 2 — National & hybrid jurisprudence.** The denser corpus and the database's real contribution: 198 prosecutions of international crimes before national and hybrid courts across 35 prosecuting countries, one row per case, with the full metadata used by the interface:
...
- cross-references to TRIAL International, UJAR, the CFJ Tool and Amnesty International.
...
- **XLSX** — the untouched original export, including its own README sheet with row counts.
